...
@Library(['camunda-ci', 'cambpm-jenkins-shared-library@CAM-12865-error-handling']) _
  ...
  stages {
    ...
    stage('UNIT DB tests') {
      matrix {
        ...
        when {
          expression {
            cambpmIsNotFailedStageType(failedStageTypes, env.PROFILE) && (withLabels(cambpmGetLabels(env.PROFILE)) || withDbLabels(env.DB))
          }
          beforeAgent true
        }
        agent {
